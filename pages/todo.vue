...
<template>
  ...
  <div class="menu bg-base-200 rounded-box">
    ...
    <p class="menu-title text-red-400 text-xl" v-if="todos.length === 0">Todoが登録されていません。</p>
    <ul>
      <li v-for="(todo, index) in todos" :key="index">
        <div class="form-control">
          <label class="cursor-pointer label gap-x-4">
            <input type="checkbox" class="checkbox checkbox-accent" v-model="todoChecked" :value="todo" />
  ...
  <div role="alert" class="alert alert-error mt-8 cursor-pointer" v-show="showAlert" @click="closeAlert">
